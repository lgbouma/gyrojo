...
\begin{abstract}
  ...
  of a uniform star formation history.  We find that the scarcity of
  sub-gigayear systems can be attributed to the star formation rate in
  the Kepler field \replaced{dropping}{smoothly decreasing} by a factor of \replaced{\ratiosfr$\pm$\uncratiosfr}{a few}
  over the past 3\,Gyr.  We observe this trend both for known planet
  hosts and for the parent stellar sample.
  %
  This ``demographic cliff'' in the Galaxy's star formation history
  has been previously reported, and its confirmation helps clarify the
  age distribution of the known transiting exoplanets.
  %
\end{abstract}
...
\begin{enumerate}[label={\it \roman*)},leftmargin=12pt,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  ...
  \item Using open cluster data, we can marginalize over the range of
    ...
    calibrations.
  %
  \item We can identify binary stars \added{and high-metallicity outliers }with greater fidelity than
...
likely precludes any such interpretation.}
...
Most \deleted{of the }stars observed by Kepler are in the thin disk.  This can
...
\replaced{For all stars in the KIC, this yields }{These criteria yield }95{,}694 thin and 54{,}039 thick
disk stars \replaced{respectively}{observed by Kepler}.  The thin disk stars have a threefold larger
...
\deleted{gyrochronology }quality flags discussed in Section~\ref{subsec:flags}
...
\replaced{those studies is their improved accuracy}{previous work is that they are demonstrably accurate} at $<$1\,Gyr\added{ (see Figure~\ref{fig:agescalecompone})}.
...
metallicity, differential rotation, mergers, and tidal spin-up.  Observational
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  ...
  \item Rather than being uniform, the age distributions of both the
    ...
    than it was three billion years ago\added{.  The statistical
    uncertainty on this value is dwarfed by the systematic
    uncertainty caused by the potential presence of unresolved
    photometric binaries (Section~\ref{subsec:caveats}); the star
    formation rate could have decreased by a factor of as much as
    4.81$\pm$0.26 over this time interval}.  \replaced{This}{Either} result \replaced{from
    rotation-based ages broadly agrees}{is generally consistent} with recent reports of a
    declining star-formation rate from CMD fitting and white-dwarf
    chronology, though with the \added{potential }advantage of better \replaced{sensitivity}{accuracy} for
    FGK stars at $t$$<$1\,Gyr (e.g.~Figure~\ref{fig:agescalecompone}).
  %
  \item Rather than expecting $\approx$500 exoplanets younger than one
    billion years, this age distribution of stars in the Kepler field
...
\acknowledgements
We thank the reviewer for their suggestions, which significantly improved the 
