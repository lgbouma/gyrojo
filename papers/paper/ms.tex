...
\begin{figure}[!t]
	\begin{center}
		\subfloat{
			\includegraphics[width=0.45\textwidth]{st_params_M_G_vs_dr3_bp_rp.pdf}
		}
	
		\vspace{-0.3cm}
		\subfloat{
			\includegraphics[width=0.45\textwidth]{st_params_adopted_logg_vs_adopted_Teff.pdf}
		}
	\end{center}
	\vspace{-0.5cm}
...
\begin{figure*}[!t]
    ...
    rotation sequences of open clusters}{seventh-order polynomials
    fitted to the slow rotation sequences of open clusters \citep[see Section~3.3 of][]{Bouma_2023}}.  The 2-D histogram of the
...
lines in this figure are the polynomials described by \citet{Bouma_2023}
that 
...
needed when assessing not only the reliability of rotational ages, but also any implications of measured age distributions.
Despite a slew of stellar quality flags, including on metallicity, evolutionary state,
and binarity, we expect that up to a third of the ``apparently clean'' 
Kepler stars with $P_{\rm rot}$$<$10\,days
could potentially be influenced by unresolved binaries (Figure~\ref{fig:hist_tgyro_corr}).
A separate nuance is in understanding that the observed age distribution does not 
directly map onto a star formation history: the kinematic heating of stars in the
disk in particular produces a major bias in the age distribution of stars, and therefore of planets,
observed by pencil-beam surveys such as Kepler.}
