...
\begin{abstract}
  ...
  over the past 3\,Gyr.  We observe this trend both in known planet
  hosts and in the parent stellar sample.
  %
...
\begin{figure}[!t]
  ...
  \caption{
    ...
    hosts to not have $Q_{\rm star}$ bits 0--\replaced{9}{10} raised, and for the
...
\begin{figure*}[!t]
	...
	\caption{{\bf Potential impact of unresolved binaries.}   This figure shows
		...
		However, even after applying our quality cuts (Section~\ref{subsec:flags}), up to
		\fivepctnuniqstarsantosrotgyroappl\ out of \ngyroappllttendays\ ``clean'' rotators with
		$P_{\rm rot}$$<$10\,days could be unresolved $\lesssim$0.5\,AU  binaries.
		...
		the sample of all Kepler stars than in the KOI host sample (see text).
...
{\it Binaries}:
\replaced{Even intermediate-separation ($\sim$10\,AU) binaries
are biased toward rapid rotation \citep[e.g.][and many studies
thereafter]{Meibom_2007}.}{Stars in binaries are often biased
...
Figure~\ref{fig:hist_tgyro_corr} shows the rotation-based age distribution
...
\citep[][]{vanSaders_2016,Hall2021,2024ApJ...962..138S}, and the observation of a ``long-period edge''
	in the $P_{\rm rot}$-$T_{\rm eff}$ distribution which
	may pile up over similar timescales \citep{David_2022}.  However, 
	this pileup is thought to happen at $>$5\,Gyr for
	late-G dwarfs and early K dwarfs \citep[see Figure~4
	by][]{2024ApJ...962..138S}.  This means that it cannot explain the
	2.3\,Gyr peak that we see in the 4400-5400\,K age distribution in Figure~\ref{fig:hist_tgyro}. 
	Independently,
	the analysis by \citet{2022ApJ...937...94M} fitted
...
This latitudinal surface differential rotation yields two separate limitations on our rotational ages.
The first is
...
$\lesssim$3\,Gyr stars that are the \added{main }focus of this work.  Specifically,
chromospheric emission in the X-ray, \ion{Ca}{2} HK line, and the UV
can serve as an age tracer
\citep{Mamajek_2008,2014MNRAS.441.2361V,2024ApJ...960...62E}.
However, these age indicators are all in a sense ``rotation-powered''.
...
rate of depletion}, $t_{\rm Li}$ and
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  ...
  \item Rather than being uniform, the age distributions of both the
    ...
    than it was three billion years ago\added{.  The statistical uncertainty on this figure is far smaller than the systematic uncertainty caused by the potential presence of unresolved photometric binaries (Section~\ref{subsec:caveats}); the star formation rate could have decreased by a factor of as much as 4.81$\pm$0.26}.  \replaced{This}{Either} result \replaced{from
...
\acknowledgements
\added{We thank the reviewer for their suggestions, which significantly improved the 
