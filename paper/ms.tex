...
\begin{abstract}
  ...
  known Kepler field clusters; they also agree with
...
exemplars include V1298~Tau, an adolescent resonant chain of four
transiting puffy planets that is a likely precusor to the compact
...
In this article we will consider planets whose existence is previously
known from transits, and infer new stellar ages using rotation and
lithium.
...
than 1\,Gyr.  Imposing a precision requirement of say $t$$<$1\,Gyr at
...
be a less significant bias than the challenge of precisely determining
...
Objects of Interest \citep[KOIs;][]{Petigura_2022}, isochrone ages are
...
Modern models yield ages precise to $\lesssim$30\% for FGK stars
between 1-4\,Gyr, and constraining age posteriors at earlier times
\citep{Bouma_2023}.  Physics-based models
\citep{Matt_2015,Gallet_Bouvier_2015,Spada_2020} are beginning to
connect these empirical relations to the evolution of stellar winds,
internal structure, and angular momentum transport.

In the context of Kepler, rotation-based ages have been reported for
planet-hosting stars since the early data releases
\citep{Walkowicz_2013,Reinhold_2015,Angus_2018}.  Recent studies
focused on rotation-based ages for Kepler have further explored
incorporating information from stellar kinematics
\citep{2021AJ....161..189L,2024AJ....167..159L}, and from light curve
features such as starspot amplitudes \citep{2023ApJ...952..131M}.  Our
analysis is motivated by a few factors that we believe constitute
improvements, particularly for the ages of sub-gigayear stars.   These
...
\begin{enumerate}[label={\it \roman*)},leftmargin=*,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  \item The Kepler Object of Interest catalog, and our vetting of
    false positives within it, has reached maturity
  ...
  \item We can fit the open cluster data, and use these fits to infer
    ...
    For Sun-like stars, this model is consistent with older
    calibrations \citep[e.g.][]{Mamajek_2008}, though with an improved treatment
    of uncertainty propagation.  For late G dwarfs and
    K dwarfs, the calibration differs by factors of a few;
  %
  \item We can identify binary stars at greater fidelity today than
  ...
  \item New open clusters in various stages of dissolution have been
    ...
    \cite[e.g.][]{2019AJ....158..122K,Bouma_2022b,2022AJ....164...88B}.
    These clusters offer a sample of stars for which we can test the
...
Sections~\ref{sec:rotage} and~\ref{sec:liage}, and test them using
stars in clusters in the Kepler field.  The population-level
...
Figure~\ref{fig:agescalecompone} shows the result of a literature
comparison against ages reported by \citet{Berger_2020a_catalog},
...
removing ages that were flagged as unreliable.
Rather than comparing the ages of the stars directly against each other,
we compared them against a common benchmark: open clusters.

Cluster memberships are a delicate subject.
Figure~\ref{fig:agescalecompone} is showing putative ages for
a set of spatially and kinematically selected candidate cluster members.  For NGC6811 and NGC6819, we
adopted candidate members from
\citet{2018A&A...618A..93C,CantatGaudin_2020} and
\citet{Kounkel_2020}. 
For Melange-3 we used the candidate members reported by
\citet{2022AJ....164...88B} and required ``offset'' tangential
...
These lists almost certainly include non-members, in fractions that we
do not aim to quantify.  Even with these contaminants, we can compare
the relative age distributions derived by the different studies.

There are two main metrics for success in this test: the first is
whether the reported ages agree with the cluster age (accuracy).  The
second is whether the reported age precisions agree with their
dispersion around the cluster age.

Starting with the isochrone ages from \citet{Berger_2020a_catalog},
the results should not be particularly surprising.
The isochrone ages are most accurate and precise (in relative terms)
for the oldest cluster, NGC6819.
For the 1\,Gyr NGC6811, about half the stars have ages overestimated
by a factor of three.
For the youngest clusters, the disagreement is greater.
The uncertainties seem accurate at 2.5\,Gyr, but perhaps less so at
$\lesssim$1\,Gyr.
An important virtue of these isochrone ages is that they are
calculated for far more stars in the two largest clusters than the
other methods.
...
The rotation-only ages calculated using the model described by
\citet{2024AJ....167..159L} have some peculiarities compared against
cluster ages.
While NGC6811 seems to be in general agreement, NGC6819 has an
abnormally large population of young reported systems, with apparently
precise ages.  All the younger cluster members ($<$300\,Myr) similarly
have reported ages that are biased old, with narrow uncertainties.
...
\begin{figure*}[!t]
    ...
    We used memberships from spatio-kinematic estimates in the
    literature.
    Rows show the available stellar ensembles, and columns show the
    various age-measurement techniques described in
    Appendix~\ref{app:litagecomparison}.
    Stars are sorted according to their median reported ages, and
    clusters are sorted by descending richness.
