...
\title{Ages of Stars and Planets Observed by Kepler}
...
%		}
	...
	\caption{
		...
		Planets highlighted in this plot have ages $t$<1\,Gyr at $2$$\sigma$.
%		
...
%		Circles are previously reported measurements from the literature.
		Stars denote systems that are newly characterized in this work; circles
		are planets from the literature meeting the same requirement.
		Gray points are transiting planets from the NASA Exoplanet Archive older than
		1\,Gyr.
		The selection function matters:
		if we were to require $t$<1\,Gyr and $t/\sigma_{\rm t}>3$, we would loose $\approx$ten
...
\begin{deluxetable*}{lllllllrrrrrrl}
  ...
  \tablecaption{Ages of Kepler planets and planet candidates.  Rows
  ...
  This version of the table is truncated to planets younger than
  one billion years.  The full machine-readable table contains ages
  and age limits for \nnonfopkoissomeageinfo\ non-false positive KOIs
  with MES$>$10.  The bit quality flags for the rotation-based ages,
  ...
  As an example, Kepler-1627 is flagged with bit 10 and bit 7.
  \label{tab:planets}}
	%\toprule
	%\midrule
	%\endhead
	\startdata
  KOI & Kepler &  $T_{\rm eff}$ & $P_{\rm rot}$ & EW$_{\rm Li}$ & $t_{\rm gyro}$ & $t_{\rm Li} $ & $R_{\rm p}$ & $P_{\rm orb}$ & $f_{\rm grazing}$ & $Q_{\rm gyro}$ &  Spec? & Comment \\
  -- &   -- & K & days & m\AA & Myr & Myr &    Earths &    days &       bool  & int & bool & -- \\
	\hline
