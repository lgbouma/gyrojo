...
\begin{abstract}
  ...
  of a uniform star formation history.  We find that this dearth can
  ...
  has been previously reported, and it helps clarify the relative
  balance of young and old planets that have been detected to date
  using transits.
  %
\end{abstract}
...
\begin{enumerate}[label={\it \roman*)},leftmargin=12pt,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  ...
  \item We can fit the open cluster data, and use these fits to infer
    ...
    This represents an improvement in the accuracy and realism
...
Figure~\ref{fig:stellarprops}.  Some of the challenges this introduces
for rotation-based ages include unresolved binaries, metallicity,
reddening, and spin-evolution effects other than magnetic braking.
...
as a way of flagging unresolved binaries, as well as mildly evolved
...
be reliable.  While this $\log g$--$T_{\rm eff}$ cut flags about half
...
BP}$-$G_{\rm RP}$ remained after applying this cut.  Such stars are
likely also questionable.  We therefore also
fitted a polynomial to the KOI main sequence, and flagged stars more
than 1\,magnitude from this locus as part of the same bitflag.
...
visually identified by \citet{1864RSPT..154....1H}.  Gaia
...
age distributions of all the stars (left column) and the
KOI hosts (middle column).  These ``histograms'' are sampled from
...
-3.9 to -1.2 for the K dwarfs, and -5.2 to -2.2 for all Kepler stars.
This agrees with the visual impression that while small differences
may be present, they are not drastic.

The similarity of the age distributions between KOI hosts and Kepler
stars adds some nuance to the argument that young transiting planets
are hard to detect due to the photometric variability of their host
stars.  If true, this statement seems to only hold for the very youngest
($\lesssim$0.4\,Gyr) stars, where there is a marginal deficit in the KOI host
age distribution relative to that of the overall stellar sample.
Examining the planet properties as a function of time, we similary
find that fewer systems at $<$0.4\,Gyr are detected with $P_{\rm
orb}$$\gtrsim$30\,days than at $>$0.4\,Gyr.
...
If this decrease continues linearly into the future, then
a simple extrapolation would imply
that in $\approx$1.4$\pm$0.3\,Gyr the rate of new stars forming in the
Galaxy will reach zero.
...
to be members of an association called Theia~520.  The core of this
...
\citet{2019AJ....158..122K,2024A&A...681A..13F}, which is within the
1$\sigma$ statistical uncertainties of our $t_{\rm gyro}$
...
Classifying Kepler stars as thin vs.~thick disk members helps connect them to 
previous work on the star formation history of the
...
they are simply flagged as not being near the main sequence in the $\log g$ vs.~$T_{\rm eff}$ plane. 
