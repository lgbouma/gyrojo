...
equivalent width of the \ion{Li}{1} 6708\,\AA\ doublet.  For this
component of the work, we rely on spectra acquired using the High
...
and 2024, primarily through the California Kepler Survey
...
These stars were chosen to ensure that confirmed planets with
rotational evidence for ages below 1\,Gyr received spectra, since this
is the age range where lithium is most likely to yield useful age
constraints.
...
%FIXME FIXME TODO LEFT OFF HERE
%FIXME FIXME TODO LEFT OFF HERE
%FIXME FIXME TODO LEFT OFF HERE
...
\citep{Bouma_2023}.  We designed this method to address
...
inferred.  The three main assumptions behind the \texttt{gyro-interp}
framework are as follows.  {\it 1)} The stars are near the main
sequence, {\it 2)} their spindown is not influenced by binary
companions, and {\it 3)} the rotation period distribution for field
stars of a given mass and age is identical to that of equally aged
open clusters (metallicity differences, for instance, are ignored).  
Regarding the former two assumptions: pre-main-sequence contraction and
post-main-sequence expansion influence surface stellar rotation periods
in ways that we do not model.  Stars in e.g.~ close binaries might be
subject to tidal effects that we do not treat.
...
\begin{figure*}[!t]
    ...
    clusters younger than 4\,Gyr.  The displayed points assume an
    unphysical uniform distribution in temperature for visual clarity.
...
flag stars for which we suspect these ages may not in fact be valid,
we then build a set of bit-quality flags.  When and how these flags should
be applied depends on the question being asked.  If the goal is to
...
grazing orbit \citep{2014ApJ...783...53M}.  Our quality flags enable
selecting for or against such cases by selecting from the following
bits.
...
\begin{figure}[!t]
    ...
    from Keck/HIRES spectra collected between 2009 and 2024.} Lines
    are the ``mean'' isochrones from the same models as in
    Figure~\ref{fig:models}.  The logarithmic scale highlights the
    dynamica range of both detections and upper limits.
		\label{fig:li_vs_teff}
	}
\end{figure}

We queried the internal CPS archive to collect all blaze-corrected
HIRES spectra that were available for the
\nplhoststarwgyroagewithgrazingandhighruwe\ stars for which
rotation-based ages can be derived.  This query yielded at least one
spectrum for \nlithiumstars\ and \nlithiumplanets\ planets.  These
spectra were acquired between 6 September 2009 and 16 May 2024.  For
...
We measured the lithium equivalent widths following a procedure from
previous work \citep{Bouma_2021}.  The stars of interest for this work
are FGK stars, and so the continuum in the vicinity of the \ion{Li}{1}
6708\,\AA\ doublet is well-defined.  We therefore numerically
integrate a single best-fit Gaussian over a local window, and
estimated uncertainties on the line width through a Monte Carlo
procedure that bootstraps against the local scatter in the continuum.
This approach does not correct for the neighboring \ion{Fe}{1}
6707.44\,\AA\ blend.
...
with those reported by \citet{2018ApJ...855..115B}.
