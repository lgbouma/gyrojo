...
\author{coauthors, including but not limited to
...
\begin{abstract}
  ...
  and \nplwgyroagenograzing\ planets for which our gyrochronology
  model is applicable; these rotation-based ages are over an order of
  magnitude more precise than isochrone ages in this sample of stars.
  %
  The independent lithium ages agree with the rotation-based ages in
  ...
  at 3$\sigma$).
  %
  More broadly, we find that the star formation rate in the Kepler field
  has dropped by a factor of \ratiosfr$\pm$\uncratiosfr\ over the past
  3,Gyr. This trend holds true for both planet hosts and the overall
  stellar sample.
  %
  This ``demographic cliff'' is a known feature of the Galaxy's star
  formation history, and it helps clarify the relative balance of
...
Exoplanet science is enjoying an age of abundance, fueled by the discovery
...
Young ($<$1\,Gyr) exoplanets represent one means of building the timeline for
...
solutions \citep[e.g.][]{2008Sci...322.1348M,2012ApJ...756L..33Q,2024AJ....167..193T}.  In
...
uniformly distributed from 0--10\,Gyr \citep[e.g.~Fig~17
...
currently forming protoplanets \citep{2018A&A...617A..44K}, and of
...
nearby sub-gigayear stars are in the field.  In other words,
$\lesssim$1\% of stars within the nearest $\approx$500\,pc have been
associated with their birth cluster
...
$\approx$5{,}000 planets.  
Barring a major discovery bias against sub-gigayear planets,
a constant star formation history would imply that an order of magnitude
...
that uniform is wrong by a factor of a few, and that increased stellar
activity for young stars may be a less important bias than the
challenge of determining precise ages for field stars.

%FIXME these paragraphs need improvements in how they incorporate
%Lu+21 (meh), but certainly Mathur+2023.
...
Beyond rotation-based ages, recent years have also yielded
improvements in the lithium age scale.  Lithium ages include two
distinct regimes: depletion boundary ages for M dwarfs and brown dwarfs in star
...
\citet{Jeffries_2023}, who modeled the evolution of 
the \ion{Li}{1} 6708\,\AA\ doublet equivalent width (EW) through
time using a set of 6{,}200 stars in 52
...
\ion{Li}{1} 6708\,\AA\ doublet.  Specifically, we analyzed spectra
...
function are different.  We therefore perform our own line width
measurements on
...
available at varying ranges of signal-to-noise (S/N), we analyzed only
...
%and convoluted,\footnote{In practice, for K dwarfs we fitted an
...
%F and G dwarfs to that of the K dwarfs.}
...
\begin{figure*}[!t]
  \begin{center}
    \leavevmode
    \subfloat{
    \includegraphics[width=.98\textwidth]{comp_hist_samples_koi_gyro_ages_hist_field_gyro_ages_20240430_maxage4000.pdf}
    }

	\vspace{-0.35cm}
    \subfloat{
    \includegraphics[width=.98\textwidth]{comp_hist_samples_koi_gyro_ages_hist_field_gyro_ages_20240430_maxage4000_preciseagesonly.pdf}
    }
  \end{center}
  \vspace{-0.6cm}
...
The stellar ages and planetary properties of the systems discussed in
the previous sections are given in Tables~\ref{tab:planets}
and~\ref{tab:stars}.  Figure~\ref{fig:hist_tgyro} shows the gyrochone
...
from each posterior distribution, and then computing the re-weighted
...
We refer to the overall slope as a ``demographic cliff'' in reference
to an impending human phenomenon forecasted to happen in coming years.

The age distributions of Kepler stars and planet hosts are
statistically quite similar.  A simple diagnostic of whether the two
...
In this work, we were interested in not only the overall age
distribution of the Kepler field, but also in the reliability of
individual ages for young stars known to host planets.  The quality
bitmask scheme in Table~\ref{tab:planets} provided one path toward
this end.  Our other method was to compare the cases for which lithium
and rotation overlap in sensitivity -- G and K dwarfs with
$t$$\lesssim$1\,Gyr.  Our results for such stars include
\ltonegyrhighqconfirmedtwosided\ planets orbiting ``high-quality''
stars for which the two age indicators are both constraining and in
agreement (stars with $Q_{\rm star}$ bits zero through nine not
raised, and no planet quality flags raised).  Conversely, we found
only one system, Kepler-786, with a precise lithium age
...
for Kepler planets, surveys such as K2, TESS, HAT, WASP, NGTS, ZTF,
and ATLAS have different selection functions.  Applying rotation-based
age dating to their discoveries could clarify the youth and
evolutionary trends of these populations.  Future prospects like PLATO
\citep{Rauer14}, Earth 2.0 \citep{2022arXiv220606693G}, and the Roman
Galactic Plane Time Domain Survey seem poisted to further expand the
sample of stars with precisely determined ages, including those known
to host planets.
...
\appendix
\section{Literature Age Scale Comparison}
\label{app:litagecomparison}

Figure~\ref{fig:agescalecompone} shows the result of a literature comparison against
ages reported by \citet{Berger_2020a_catalog}, \citet{2021AJ....161..189L}, 
 \citet{2023ApJ...952..131M}, and
\citet{2024AJ....167..159L}.
We followed any guidance available from these respective studies for removing ages
that were flagged as unreliable.
Rather than compare the ages of the stars directly against each other, we then decided
to compare them against a common benchmark: open clusters.

Cluster memberships are a delicate subject.
Figure~\ref{fig:agescalecompone} is in fact showing putative ages for a set of putative cluster members.
For NGC6811 and NGC6819, we adopted all reported members from \citet{2018A&A...618A..93C,CantatGaudin_2020} and \citet{Kounkel_2020}. 
For Melange-3 we used the members from \citet{2022AJ....164...88B} and required ``offset'' tangential velocities below 2\kms.
For Cep-Her we used stars from Table~2 of \citet{Bouma_2022b}, requiring ``weights'' exceeding 0.1.
These cluster lists almost certainly include non-members, in fractions that we do not aim to quantify.
Even with these contaminants, we can compare the relative age distributions derived by
the different studies.

There are two main metrics for success in this test:
the first is the extent to which the reported ages agree with the cluster age (accuracy).
The second concerns how precise the reported ages are, and whether that precision agrees with the observed
distribution of ages around unity.

Starting with the isochrone ages from \citet{Berger_2020a_catalog}, the results should not be particularly surprising.
The isochrone ages are most accurate and precise (in relative terms) for the oldest cluster, NGC6819.
For the 1\,Gyr NGC6811, about half the stars have ages overestimated by a factor of three.
For the youngest clusters, the disagreement is greater.
The uncertainties seem accurate at 2.5\,Gyr, but perhaps less so at $\lesssim$1\,Gyr.
An important virtue of these isochrone ages is that they are calculated for far more stars in the two largest
clusters than the other methods.
They are calculable for far more stars than rotation-based ages.

The ``gyro-kinematic'' ages from \citet{2021AJ....161..189L} do...
{\bf todo explain}

The rotation-only ages calculated using the model described by \citet{2024AJ....167..159L} have some
peculiarities compared against cluster ages.
While NGC6811 seems to be in general agreement, NGC6819 has an abnormally large population of
young reported systems, with apparently precise ages.  All the younger cluster members
($<$300\,Myr) similarly have reported ages that are biased old, with narrow uncertainties.

\citet{2023ApJ...952..131M} looks great.  {\bf todo explain method}.
However it still fails for the youngest clusters.
...
\begin{figure*}[!t]
	\begin{center}
		\subfloat{
			\includegraphics[width=0.98\textwidth]{merged_lit_ages_vs_cluster_ages.pdf}
		}
	\end{center}
	\vspace{-0.5cm}
	\caption{
    {\bf Literature age scale comparison.}
    Each point denotes the reported age of a star observed by Kepler,
    normalized by the age of a stellar ensemble in which the star
    is a putative member.
    Rows show the available stellar ensembles, and
    columns show the various age-measurement techniques described in Appendix~\ref{app:litagecomparison}.
    Stars are sorted according to their median reported ages.
    Note that the horizontal axis is logarithmic; the first dotted line
    to the right of unity corresponds to a reported age ten times older
