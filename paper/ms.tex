...
$\log_{\rm 10}p$ values spanned (2.5$^{\rm th}$ to 97.5$^{\rm th}$ percentiles)
-3.9 to -1.2 for the K dwarfs,
and -5.2 to -2.2 for all Kepler stars.  This suggests 
that while small differences may be significant, they are not drastic.
This similarity
contradicts
the common-sense argument that young transiting planets are hard to detect
due to the photometric variability of their host stars.  If true, this statement only holds for the very youngest
($<$0.4\,Gyr)
stars, where there is a marginal deficit in the KOI host age
