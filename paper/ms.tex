...
\begin{abstract}
  ...
  stellar sample.
  %
  This ``demographic cliff'' is a known feature of the Galaxy's star formation
  history, and it helps clarify the relative balance of
  young and old planets that have been detected to date using transits.
  %
\end{abstract}
...
help build the timeline for exoplanet evolution.  Informative
...
Despite this and other observational challenges, young close-in planet discovery
has matured over the past decade, primarily due to Kepler, K2, TESS,
and Gaia
\citep[e.g.][]{Meibom_2013,Mann_K2_25_2016,Curtis_2018,Livingston_2018,David_2019,Bouma_2020_toi837,Rizzuto_2020,Plavchan_2020,Newton_2021,Nardiello_2022,Barber_2022,Bouma_2022a,Bouma_2022b,Zhou_2022,Zakhozhay_2022,Wood_2023}.
...
finding the host stellar association
...
to assume a uniform age distribution for transiting planet host star ?
Second, where are the missing young planets?  Briefly, we will find
that uniform is incorrect by a factor of a few, and that at least one
bias against the discovery of young transiting planets is in the
measurement of their ages.
...
%of $\approx$2.6\,Gyr \citep{Meibom_2015,Curtis_2020}.
...
However, beyond rotation-based ages, recent years have
also yielded improvements in the lithium age scale.  Lithium ages
include two distinct regimes: depletion boundary ages
...
suitable for measurement of the \ion{Li}{1} 6708\,\AA\ doublet
...
\begin{figure*}[!t]
	...
	\caption{
		...
		to the slow rotation sequences of open
		clusters.  The stellar sample (left) includes only apparently single stars
		near the main sequence with $\log g$$>$4.2, RUWE$<$1.4,
		and temperatures of 3800--6200\,K.
		The color map has a linear stretch.  %between one and ten stars per bin.
		The planet-hosts (right) require
...
Many investigators have derived rotation periods both specifically for
KOIs
...
functions.
We are interested in understanding the age distribution of all Kepler
target stars irrespective of KOI status.
Out of these studies, the only homogeneous analysis of both Kepler
targets and KOIs were the studies by \citet{Santos_2019,Santos_2021}
(\citetalias{Santos_2019} and \citetalias{Santos_2021}).
...
compiled from
\citetalias{Santos_2019}, \citetalias{Santos_2021}, and our extended
KOI list.
...
\ion{Li}{1} 6708\,\AA\ doublet.  Specifically,
...
effective temperatures of 3800--6200\,K.  To flag stars for
...
% of the rotation periods as a function of temperature 
...
%For instance, rapid rotators are more common in binaries.
%and they also
...
In these instances, we quote the 3$\sigma$ (99.7$^{\rm th}$
percentile) limits.
...
\begin{figure*}[!t]
  ...
  \end{center}
  \vspace{-0.6cm}
  \caption{{\bf Kepler's demographic cliff,} visible in the rotation-derived
    ages of stars (left) and planet hosts (middle) in the Kepler field.
    The top row corresponds to all
    stars with temperatures of 3800--6200\,K for which we calculated rotation-based ages.
    The darker lines impose quality cuts
    on factors such as binarity, crowding, and the star's evolutionary
    state.
    The bottom row selects only stars with temperatures of 4400-5400\,K, which
    have relatively precise ages near 2\,Gyr due to their fast spin-down
    \citep[see][]{Bouma_2023}.
    The statistical uncertainty for a mean individual star at 1, 2, and 3\,Gyr is shown, and is identical across each row.
    Finally, the right panel compares the rotation-based age distributions against star formation histories derived using CMD fitting.
    Completeness is near unity until $t$$\lesssim$3\,Gyr
...
%the dispersion of the lithium data evolve over the first gigayear.


\subsubsection{New Young Planets}
%
%The astrophysical processes behind rotation and lithium evolution are
%independent.
%Therefore 
%whenever the two age indicators agree within a
%narrowly-constrained range, confidence in the true age
%increases.
%The region of greatest overlap between rotation and lithium ages is
%dictated by the timescale for lithium depletion: for stars with
%temperatures between 4500-5500\,K with ages $\lesssim$1\,Gyr, we can
%hope to obtain independent two-sided constraints
%\citep{Bouma_2023,Jeffries_2023}.
...
%Two of the former systems are in clusters (Kepler-1643 and Kepler-1928);
%five of the latter systems are in Theia~520.
...
two transiting planets: an Earth-sized planet on a one-day orbit,
...
%though at half the relative uncertainty.
...
\citep[e.g.][and many studies thereafter]{Meibom_2007}. %,2016AJ....152..115S,2019ApJ...871..174S,Gillen_2020,Bouma_2021}.
...
Conversely, we found only one system, Kepler-786, with a 
precise lithium age (\kepseveneightsix\,Myr), but no rotation signal.
...
X-ray, \ion{Ca}{2} HK line, and the UV can serve as an age tracer
...
Rotation-based age dating of their discoveries could 
potentially clarify their youth, and thereby help clarify the broader
issue of their evolution.
...
using empirical methods that have been
...
\begin{deluxetable*}{llllllllrrrrrrl}
  ...
  A shell script to decode the $Q_{\rm star}$ quality flag is {\bf \href{https://gist.github.com/lgbouma/20368253f1a98da1b39cf32fdda0be13}{available online}}.
  \label{tab:planets}}
  %\toprule
  %\midrule
  %\endhead
...
%  \input{table_subgyr_candidate}
  ...
  \tablecomments{``Confirmed'' planets appear in the machine-readable version before``candidate'' planets.
    ...
    $Q_{\rm star}$, are described in Section~\ref{subsec:flags}.
...
\paragraph{Kepler-101}---This $\approx$5500\,K star has EW$_{\rm Li}$=100$\pm$5\,m\AA,
nominally implying $t_{\rm Li}$$\approx$200-800\,Myr, but no rotation detection.
Our automated flags noted that this star has a low surface gravity, high luminosity, and is far from the main sequence.
\citet{2014A&A...572A...2B} concur: this star is a sub-giant slightly more massive than the Sun;
no rotation detection is expected; the lithium likely simply survived over the star's
main sequence lifetime.

\paragraph{Kepler-108}---This system of two mutually
inclined giant planets \citep{2017AJ....153...45M} has a $T_{\rm eff}$$\approx$5600\,K
host star with a strong lithium detection, and no rotation detection.
The host star is entering the red giant branch; this has been previously noted
through asteroseismology \citep{2013ApJ...767..127H}, and our analysis
of the Keck/HIRES spectrum with \texttt{SpecMatch-Synth} \citep{2017AJ....154..107P} concurs, yielding
$T_{\rm eff}$=5668$\pm$100\,K, $\log g$=3.8$\pm$0.1,
[Fe/H]=0.38$\pm$0.06, and $v\sin i$=3.2$\pm$1.0\kms.
Given the star's high mass, the high lithium content and lack of rotation detection
is not surprising.
...
%
%Our quality flags noted that this star has
%a low surface gravity, a crowded field, high RUWE, and an ambiguous Kepler-to-Gaia crossmatch.

\paragraph{Kepler-786}---This K3 V star has a surprisingly 
high lithium content.
The spectrum is single-lined, with
 \texttt{SpecMatch-Synth} derived parameters of
 $T_{\rm eff}$=4769$\pm$100\,K, $\log g$=4.5$\pm$0.1,
 [Fe/H]=0.11$\pm$0.06, and $v\sin i$=2.4$\pm$0.6\kms.
Yet with 
EW$_{\rm Li}$=83$\pm$6\,m\AA,
the lithium age of \kepseveneightsix\,Myr would predict an obvious rotation signal.
None is present in the Kepler light curve, consistent with the low
$v\sin i$.
Out of all ``apparently discrepant'' lithium and rotation measurements
discussed in this appendix, this is the only one that seems to remain discrepant after scrutiny.
The  \ion{Ca}{2} doublet is in emission, with $R'_{\rm HK}=-4.7\pm0.5$, which
suggests an age at least as old as the Hyades
\citep{Mamajek_2008}.
The Balmer lines are in absorption, and display no obvious signatures of youth.
...
All are subgiants, flagged with $Q_{\rm star}$ under various combinations of bits 1, 2, and 9.
