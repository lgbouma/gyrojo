...
In the entire sample (i.e.~without imposing an quality cuts)
this yielded \allagesyesconsistent\ consistent
cases, \allagesmaybeconsistent\ ``maybe consistent'' cases, and
...
systems for which at least one age indicator
reports a sub-gigayear age in detail.
All but one are simply cases for which the star
is either evolved or else an unresolved binary.
The one exception is Kepler-786 (KOI-1199), an early K dwarf with 
$t_{\rm Li}$=\kepseveneightsix\,Myr, which would predict an obvious rotation signal.
No photometric signal is detected; other age indicators in the
spectrum similarly suggest that the star may not in fact be young.
