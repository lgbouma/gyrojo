...
\begin{figure}[!t]
	\begin{center}
		\leavevmode
		\includegraphics[width=0.47\textwidth]{li_vs_teff_koi_X_JUMP_eagles_logy.pdf}
	\end{center}
	\vspace{-0.6cm}
	\caption{
...
\begin{figure*}[!t]
  ...
  \caption{{\bf Kepler's demographic cliff,} visible in the rotation-derived
    ...
    stars with $T_{\rm eff}\in[3800,6200]\,$K for which we calculated rotation-based ages.
    The darker lines impose quality cuts
    on factors such as binarity, crowding, and the star's evolutionary
    state.
    The bottom row selects only stars with $T_{\rm eff}\in[4400,5400]\,$K, which
