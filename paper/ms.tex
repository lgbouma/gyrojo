...
% <250 words
\begin{abstract}
  Recent studies of stars in open clusters have yielded detailed
  pictures of how the rotation rates and lithium content of FGK stars
  evolve during the first few billion years of their lives.
  %
  ...
  gyrochronology is probably valid.
  %
  The independent lithium ages show X,Y,Z, though they are subject to their own systematic issues.
  %
  ...
  this sample,
  we report new robust ages from both rotation and lithium for 14
  confirmed $<$500\,Myr planets, including five mini-Neptunes, five
  super-Earths, three Earths and sub-Earths, and a Neptune-sized
  planet.
  %
  In the broader Kepler target star sample, we find that the
  gyrochrone age distribution rises linearly from 0 to 3\,Gyr, with
  stars 2\,Gyr old being twice as common in the Kepler field as stars
  1\,Gyr old.
  %
  The resulting age distribution bounds the relative balance of
  ``young'' and ``old'' planets detectable not only by Kepler, but
  also by future wide-field transit surveys.
  %
...
of thousands of worlds orbiting close to their host stars (CITE, CITE,
CITE).
However, most known exoplanets have ages comparable to that of the solar
system.
This unfortunate fact leaves many gaps in our knowledge of
exoplanetary origins, particularly regarding to degree to which
dynamical migration (CITE), thermal cooling (CITE), and atmospheric
loss (CITE) sculpt planetary systems after the disk disperses.
Finding and characterizing young ($<$1\,Gyr) exoplanets offers one
possible workaround, since these objects can show both individual (CITE,
CITE) and population-level (CITE) evidence for these types of
...
of stars.  The lithium technique will provide useful information for
...
%		}
	...
	\vspace{-0.5cm}
	\caption{
    {\bf The stellar samples.}  Our analysis focuses on stars observed
    ...
    that meet an additional set of quality criteria (blue points;
    Section~\ref{subsec:plflags}).  These are the objects for which
    our rotation-based ages are most reliable.  We also report
    lithium-based ages for a largely-overlapping setting of KOIs based
    on high-resolution spectroscopy.  Surface gravities and effective
...
The three main assumptions required for a rotation-based age dating
method like \texttt{gyro-interp} to be valid are as follows.  {\it 1)}
The target star must be near the main sequence, {\it 2)} its spindown
must not be influenced by binary companions, and {\it 3)} the rotation
period distribution for field stars of a given mass and age must be
identical to that of equally aged open clusters (metallicity
differences, for instance, are ignored).  Regarding the former two
assumptions: pre-main-sequence contraction and post-main-sequence
expansion influence surface stellar rotation periods in ways that we
do not model.  Stars in e.g.~ close binaries might be subject to tidal
effects that we do not treat.
...
over 500 grid points.  \texttt{gyro-interp} itself is calibrated
out to 4\,Gyr, the adopted age of the M67 cluster
\citep[see][]{2022ApJ...938..118D,Gruner_2023}.  To estimate the stellar
spin-down rate as a function of time and stellar temperature, the
...
greatly toward Kepler's noise floor for planet detection (CITE).  We
required a S/N in excess of the usual floor, through a cut on the
...
\begin{figure}[!t]
	...
	\vspace{-0.5cm}
	\caption{
    {\bf Rotation periods for Kepler stars (top) and planets
    (bottom)}.  The periods are from \citet{Santos_2019} and
...
age, spanning $10^6$ to $10^{10.1}$\,yr, and symmetrize our EW
...
crowding and binarity since those effects can produce both systematic
and astrophysical biases in EW measurements.
...
\begin{figure*}[!t]
	...
	\vspace{-0.6cm}
	\caption{
		{\bf Age distribution of stars (left) and planet hosts
...
\begin{figure*}[!t]
	...
	\caption{
		...
		Stars denote \nplyounggyrotwosigmanograzingnoruwe\ systems that are newly characterized in this work to have
		$t_{\rm gyro}$<1\,Gyr at $2$$\sigma$; circles
		are planets from the literature meeting the same age requirement, but with a heterogeneous
		set of age provenances.
		Gray points are transiting planets from the NASA Exoplanet Archive older than
		1\,Gyr.
		The selection function matters:
		selecting for precise ages ($t/\sigma_{\rm t}>3$) would hide $\approx$ten
		$<$0.5\,Gyr planets, and add $\approx$twenty 0.5-1\,Gyr planets.
		For this plot, we require our Kepler stellar hosts to have no quality flags raised,
		and for the planets to similarly no have quality flags raised. 
		Weakening these constraints would expand the sample at the expense of reliability.
		\label{fig:rp_period_age_results}
	}
\end{figure*}
...
\startlongtable
\begin{deluxetable*}{lllllllrrrrrrl}
	%\tabletypesize{\footnotesize}
	\tabletypesize{\scriptsize}
  \tablecaption{Ages of Kepler planets and planet candidates. 
  This version of the table is truncated to include the youngest fifty confirmed Kepler planets, and the twenty youngest candidates.
  The full machine-readable table contains ages
  and age limits for \nnonfopkoissomeageinfo\ non-false positive KOIs
  with MES$>$10.  
  \label{tab:planets}}
	%\toprule
	%\midrule
  ...
  $t_{\rm gyro}$ & $t_{\rm Li} $ & $R_{\rm p}$ & $P_{\rm orb}$ &
  $Q_{\rm planet}$ & $Q_{\rm gyro}$ &  Spec? & Comment \\
  -- &   -- & K & days & m\AA & Myr & Myr &    Earths &    days &       int  & int & bool & -- \\
	\hline
...
%	\rule
	...
	\enddata
	\tablecomments{Rows
		are sorted by the minimum of either the rotation or lithium-based
		age, and ``confirmed'' planets appear before``candidate'' planets.
...
\begin{deluxetable*}{lllllllrrrrrr}
	%\tabletypesize{\footnotesize}
	\tabletypesize{\scriptsize}
  \tablecaption{Ages of Kepler target stars.  
  The full machine-readable table contains rotation-based ages
  for \nuniqstarsantosrotteffcut\ stars with temperatures spanning
  3800-6200\,K.
  \nuniqstarsantosrotgyroappl\ of these stars have quality flags
  without bits 0 to 9 raised, meaning that they are likely valid
  objects for gyrochronology (see Section~\ref{subsec:flags}).
  \label{tab:stars}}
	%\toprule
	%\midrule
	%\endhead
	\startdata
  KIC & Gaia &  $T_{\rm eff}$ & $P_{\rm rot}$ & $t_{\rm gyro}$ & $Q_{\rm gyro}$ \\
  -- &   -- & K & days &  Myr &    int  \\
	\hline
  %FIXME make it!
	%\input{table_gyrostars} %\\
	%\hline
	\enddata
