...
\begin{abstract}
  ...
  star formation history.  We find that this dearth can be attributed to the star formation
  rate in the Kepler field
  dropping by a factor of \ratiosfr$\pm$\uncratiosfr\ over the past
  3\,Gyr.  This trend is observed both for known planet hosts as well as for the overall
  stellar sample.
  %
...
\begin{figure}[!t]
	\begin{center}
		\subfloat{
			\includegraphics[width=0.475\textwidth]{st_params_M_G_vs_dr3_bp_rp.pdf}
		}
	
		\vspace{-0.3cm}
		\subfloat{
			\includegraphics[width=0.475\textwidth]{st_params_adopted_logg_vs_adopted_Teff.pdf}
		}
	\end{center}
	\vspace{-0.5cm}
...
few $\gtrsim$1\,Gyr open clusters known in the Kepler field had been
...
ages, as well as select ages from the literature, against the ages of
stars in the richest clusters in the Kepler field.  From the
...
followed any guidance available from these respective studies for
removing ages that were explicitly flagged as unreliable, and plotted
stars within $1\sigma$ of zero as being upper limits.  For the
\citeauthor{Reinhold_2015} ages, we used those that were calculated
...
\citep{Amard2019}, based on the defaults adopted by
\citeauthor{2023ApJ...952..131M}.

Cluster membership is a delicate subject.
Figure~\ref{fig:agescalecompone} is showing reported ages for a set of
spatially and kinematically selected set of stars that could be
...
\citet{Bouma_2022b}, requiring ``weights'' exceeding 0.1.  Even with
...
There are two main metrics for success in this test: accuracy in
whether the reported ages agree with the cluster age, and precision in
whether the reported age uncertainties agree with their dispersion
...
stars $\gtrsim$1\,Gyr old, the ages of sub-gigayear stars have
historically been overestimated by anywhere from 0.3--2\,dex.  The
situation is particularly severe for the 40\,Myr stars in Cep-Her, but
it is also present for 150--300\,Myr stars in Melange-3 and Theia~520.
For isochrone and kinematic ages, this is not surprising because these
methods rely on parameters that do not appreciably change at
$t$$\lesssim$1\,Gyr.  For the rotation-based ages quoted by
\citet{2023ApJ...952..131M} and \citet{2024AJ....167..159L}, the
discrepancy is likely
caused by details of the calibration methodologies.  The physical
spin-down models used by \citet{2023ApJ...952..131M}, and the
gyro-kinematic calibration ages used by \citet{2024AJ....167..159L},
may not reproduce the rotation sequences of open clusters such as
Praesepe, the Hyades, and the Pleiades.  The \citet{Mamajek_2008}
calibration used by \citet{Reinhold_2015} does reproduce such
sequences for Sun-like stars.  However the
uncertainties determined using this approach are underestimates, because
they do not marginalize over
the intrinsic scatter in rotation periods present for stars of a fixed
mass and age.
...
%  \input{table_subgyr_candidate}
  ...
  \tablecomments{
  	...
  	Two checkmarks ($\checkmark\checkmark$) denote systems with two-sided $t_{\rm
  gyro}$ and $t_{\rm Li}$ for which both the age and the planet are expected to be reliable.  One checkmark ($\checkmark$) denotes two-sided $t_{\rm gyro}$ only. 
