...
Stars that formed in the same birth cluster, at approximately the same time,
are the gold standard for the astronomical age scale
\citep{Soderblom10}.
Before Gaia, only a handful of $\gtrsim$1\,Gyr open clusters
were known in the Kepler field.


The gold standard for stellar ages comes from open clusters
...
\begin{figure*}[!t]
	\begin{center}
		\subfloat{
			\includegraphics[width=0.98\textwidth]{olympic_merged_lit_ages_vs_cluster_ages.pdf}
		}
	\end{center}
	\vspace{-0.5cm}
	\caption{
		{\bf Literature age comparison.}
		Each point denotes a reported age,
		normalized by the age of a stellar ensemble in which the star
		is a candidate member.
		We used candidate cluster members as reported by previous studies based on spatial and kinematic
		...
		this fixed cluster list; certain methods report ages for more stars than others.
