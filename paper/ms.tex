...
\begin{abstract}
  ...
  gyrochronology is valid; these rotation-based ages are roughly an
  order of magnitude more precise than isochrone ages in this sample
  of stars.
  %
  ...
  field has dropped by a factor of \ratiosfr$\pm$\uncratiosfr\ over
  the past 3\,Gyr, equally for both planet hosts and for the overall
  stellar sample.
  %
...
studies by \citet{Santos_2019,Santos_2021} (\citetalias{Santos_2019}
...
{\it Occurrence Rates}:
Some interesting age-evolution trends have been predicted, and can
probably be teased from our data.  Kelvin-Helmholtz cooling for
mini-Neptunes should scale $R_{\rm p}$ as $t^{-0.1}$
\citep{Gupta_2019}.  The photoevaporation desert has been predicted to
be ``carved'' over the timescale for atmospheric loss
\citep{Owen_Lai_2018}.  Mini-Neptunes should be converted into
super-Earths \citep{Rogers_2021}.  And the boundary of the super-Earth
population may correspondingly move up in size over time
\citep{David_2021}.  We're going to split the two papers.

{\it Field star ages from other surveys}:
In this study we focused on deriving rotation and lithium-based ages
for the Kepler planets.  However surveys such as K2, TESS, HAT, and
WASP have had very different selection functions.  Rotation-based age
dating of their discoveries could potentially clarify their youth, and
thereby help clarify the broader issue of their evolution.

{\it Searches for new associations}:
Some of the youngest systems in Table~\ref{tab:planets} have been
connected to their birth clusters.  Others have not.  A systematic
search for the birth clusters of the youngest ``field stars'' has, to
our knowledge, not been performed.  The ideal approach for such a
search would be to combine spatial and kinematic information from Gaia
with rotation measurements from TESS.  Given that the decoherence time
of young clusters is $\sim$100\,Myr, one would expect that the
youngest ``field stars'' may have young neighbors that are relatively
easy to identify.
