...
We measured the lithium equivalent widths following a procedure
...
broadly consistent).  We then continuum-normalized by fitting a
...
\begin{deluxetable*}{lllllllrrrrrr}
	...
	\tablecaption{Ages of Kepler planets and planet candidates.  Rows are sorted by the minimum of either the rotation or lithium-based age; this version of the table is truncated to planets younger than one billion years.  ``Confirmed'' planets appear before``candidate'' planets.
		The full machine-readable table contains ages and age limits for \nnonfopkoissomeageinfo\ non-false positive KOIs with MES$>$10.
		The bit quality flags for the rotation-based ages, $Q_{\rm gyro}$, are described in Section~\ref{subsec:flags}.
