...
%  \input{table_subgyr_candidate}
  ...
  not be raised, and for a planet to be ``reliable'', we suggested
  $Q_{\rm planet}$ to be zero.
  In the comment string, a single \checkmark\ denotes systems with no
  reliability issues for which $t_{\rm gyro}$$<$1\,Gyr but $t_{\rm
  Li}$ is single-sided; a double \checkmark\checkmark\ denotes
  analogous systems with two-sided rotation and lithium ages in
  agreement.
  }
\end{deluxetable*}
