...
\begin{abstract}
  ...
  3\,Gyr.  We observe this trend both for known planet hosts and also
  for the parent stellar sample.
  %
...
Discovering a young planet requires solving two problems: finding the planet and measuring the star's age.
Each problem admits a range of
solutions
...
than 1\,Gyr.  Requiring $t$$<$1\,Gyr at
...
them from the main sequence.  For sub-gigayear stars on
the main sequence, isochrone ages are therefore limited.
...
between 1-4\,Gyr, and constraining age posteriors for younger ages
...
factors that can yield improvements, particularly for 
...
clusters in the Kepler field.  We discuss population-level
trends in Sections~\ref{sec:results} and~\ref{sec:disc}, and offer a
few conclusions in Section~\ref{sec:conclusions}.
...
\begin{figure}[!t]
  ...
  sequence (orange line in lower panel; see Section~\ref{subsec:flags}).  Some
...
\begin{figure*}[!t]
    ...
    planet hosts (right)}.  
    The gray lines are ``mean fits'' to the rotation sequences of
    open clusters.  The 2-D histogram of the stellar sample (left) includes only apparently
    single stars near the main sequence with $\log g$$>$4.2,
    RUWE$<$1.4, and temperatures of 3800--6200\,K.  
    The planet-hosts (right) require the same
    stellar cuts, and include only the confirmed and candidate planets
    described in Section~\ref{subsec:plflags}.  The number of stars $N_{\rm s}$ and
    planets $N_{\rm p}$ are noted in 
    the relevant panels.
    A simple
    estimate for the number of young planets discovered by Kepler
    follows by counting points below the mean rotational isochrones.
	}
...
more scrutiny than those of ordinary Kepler stars.  We
...
least 10.  This yielded at least one spectrum for 1{,}464
stars hosting 2{,}174 planets.  About half of these stars have
measured rotation periods (\nstwithspecandprot\ stars and
\nplwithspecandprot\ planets, respectively).   For stars with multiple spectra
...
iteration on the \nlithiumstars\ stars with spectra, we compared our
lithium equivalent widths with those reported by
\citet{2018ApJ...855..115B}.  For the stars in both
...
\citet{Santos_2019} and \citet{Santos_2021}, which are
primarily derived from the photometric \citet{Mathur_2017} DR25 Kepler Stellar
Properties Catalog.  In the
...
be valid, we then built a set of quality flags which we condensed
...
%We set this threshold by examining the
...
Some planets are more reliably identified than others.  We used the
following additional quality indicators to assess the reliability and
utility of a planet, and assembled them into a separate bitmask,
$Q_{\rm planet}$.
...
\begin{figure*}[!t]
	...
	\caption{
		...
		Each point denotes a star's reported age,
    ...
    certain methods report ages for more stars than others.
		Horizontal scatter is added to visually
		clarify the statistical age uncertainties.
		At $<$1\,Gyr,
    our ages are generally accurate, and their statistical
    uncertainties match their dispersion.
	}
...
on isochrone, rotation, and lithium dating
...
removing ages that were unreliable, and plotted
stars within $1\sigma$ of zero as upper limits.  For the
...
the range of rotation periods accessible to sub-gigayear stars of a fixed
mass and age.
...
%    \includegraphics[width=.96\textwidth]{comp_hist_samples_koi_gyro_ages_hist_field_gyro_ages_20240521_maxage4000_preciseagesonly.pdf}
  ...
  rotation-derived age distributions of its stars (left) and planet hosts (middle).  The top row shows all stars with
  temperatures of 3800--6200\,K for which we calculated rotational
  ...
  star at 1, 2, and 3\,Gyr is shown, and is identical
...
rotational ages are calibrated.  The overall slope
...
KOI host age distribution relative to the parent stellar
...
evolution.
Our catalog has \nplyounggyro\ confirmed and candidate planets
...
above the cluster age reported by \citet{Bouma_2022b} for RSG-5.
...
%		{\bf Sizes, orbital periods, and rotation-based ages of transiting
%			exoplanets younger than one billion years.} Star symbols denote
%		\nplyounggyrotwosigmanograzingnoruwe\ confirmed and candidate planets that are 
%		characterized in this work to have $t_{\rm gyro}$<1\,Gyr at
%		$2$$\sigma$; circles are planets from the literature meeting the
%		same age requirement.  Gray points are transiting planets from the NASA
%		Exoplanet Archive older than 1\,Gyr.  If one were to instead
%		select for precise ages ($t/\sigma_{\rm t}$$>$3), this would hide
%		ten $<$0.5\,Gyr planets, and add fifteen
%		0.5-1\,Gyr planets.  For this plot, we require our Kepler stellar
%		hosts to not have $Q_{\rm star}$ bits 0--9 raised, and for the planets 
%		to similarly no have quality flags raised. 
%		\label{fig:rp_period_age_results}
...
known planet hosts 
in the ``Consistent?'' column of Table~\ref{tab:planets}.
...
galaxy, previous studies have measured star
...
focused on stars in a wider 2\,kpc bubble, and additionally
reported three local
maxima in the galactic star formation rate, which they associated with pericenter passages of the
...
youngest ``field stars'' could combine positions and velocities
from Gaia with rotation measurements from TESS. Given the
...
Tables~\ref{tab:planets} and~\ref{tab:stars} summarize the results for planets and stars,
respectively.
Our recovered ages are accurate for all 14 known Kepler planets
in clusters.  While lithium provided minimal added information
for most of the sample, $t_{\rm Li}$ and $t_{\rm gyro}$ agreed in
...
``candidate'' planets, grazing transit geometries, and stars with RUWE far from unity
expands the counts by a factor of about two.
...
%such as on transit geometry, would further increase the counts. 
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
  %
  \item Rather than being uniform, the age distributions of both the Kepler target stars and the
    ...
    the advantage of better sensitivity for FGK stars at $t$$<$1\,Gyr
    (e.g.~Figure~\ref{fig:agescalecompone}).
  %
  ...
  \item We have derived rotation-based ages for
    ...
    $\approx$170 known sub-gigayear planets.
    This lessens the original factor of ten discrepancy to
